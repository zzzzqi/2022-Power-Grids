...
plt.rc("font", size=7)                                              ## Set the font size of the charts
...
def plot_phase_space_graph(import_file, export_file, tau):
    ...
    # plot
    fig = plt.figure()
    fig.set_size_inches(2, 2)                                       ## Changed the dimensions
    plt.style.use('grayscale')  # plot the graph with grayscale
    plt.plot(pf['x'], pf['y'])
    plt.xlim(-2, 2, 1)                                              ## Added the steps in the axes
    plt.ylim(-2, 2, 1)
    plt.axis('on')                                                  ## Show the axes
    # plt.show()
    plt.savefig(export_file, dpi=100)                               ## Increase the resolutions to make it 200 * 200

    fig.clear()                                                     ## Added for memory management
    plt.close(fig)                                                  ## Added for memory management
    plt.close("all")                                                ## Added for memory management
