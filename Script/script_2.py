...
def colour_to_gray(image_name):
    colour_image = PIL.Image.open(image_name)
...
df_columns_predictions = ["event_id", "start_time", "asset_name", "manual_event_type", "input_event_csv_filename",
                          "vab_flickers", "vab_harmonics", "vab_interruptions", "vab_interruptions_harmonics",
                          "vab_osc_transients",
                          "vab_sags", "vab_sags_harmonics", "vab_spikes", "vab_swells", "vab_swells_harmonics",
                          "vbc_flickers",
                          "vbc_harmonics", "vbc_interruptions", "vbc_interruptions_harmonics", "vbc_osc_transients",
                          "vbc_sags",
                          ...
                          "ia_flickers",
                          "ia_harmonics", "ia_interruptions", "ia_interruptions_harmonics", "ia_osc_transients",
                          "ia_sags",
                          "ia_sags_harmonics", "ia_spikes", "ia_swells", "ia_swells_harmonics", "ib_flickers",
                          "ib_harmonics",
                          "ib_interruptions", "ib_interruptions_harmonics", "ib_osc_transients", "ib_sags",
                          "ib_sags_harmonics",
                          "ib_spikes", "ib_swells", "ib_swells_harmonics", "ic_flickers", "ic_harmonics",
                          "ic_interruptions",
                          "ic_interruptions_harmonics", "ic_osc_transients", "ic_sags", "ic_sags_harmonics",
                          "ic_spikes",
...
for file in files:
    if re.search(csv_suffix, file) is not None:
        ...
        path = 'input_event_samples' + os.sep + file
        signal = pd.read_csv(path, index_col=4)  # index_col means choose which col as the row labels
        ...
        for waveform in Waveforms:
            ...
            # prediction_image = PIL.Image.frombytes('RGB', fig.canvas.get_width_height(), fig.canvas.tostring_rgb())
...
#         predictions = cnn.predict(prediction_image_array)
#         for i in range(10):
#             output_file.loc[input_event_csv_filename, wave.lower() + '_' + col_names[i]] = predictions[0][
#                 i]  ### Use input_event_csv_filname as the index to identify which cell to go into
#         count += 1
#
