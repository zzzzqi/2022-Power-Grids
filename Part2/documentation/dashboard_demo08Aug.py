...
def dynamic_env(df):
    """
    The method reads the input CSV file and set it for the dynamic environment

    :param df: The input file(bytes)
    :return: the layout of the entire page, including two columns:

    #. algorithm widgets, plot configuration , similar events configuration
    #. data exploration pane.
    ...
    def plot_configuration(dr_value, clustering_value):
        """
        The method builds the WidgetBox for plot configuration, based on the selection of dimensionality reduction
        algorithm and clustering algorithm.

        :param dr_value: The value of the selection dropdown for Dimensionaity Reduction algos.(param.parameterized.Parameter)
        :param clustering_value: The value of the selection dropdown for Clustering algos.(param.parameterized.Parameter)
        :return: A widget box include data exploration pane options and parameters of algorithms.
    ...
    def similar_events_configuration(dr_value, clustering_value):
        """
        The method builds the similar events configuration widgetbox, based on the selection of dimensionality reduction
        algorithm and clustering algorithm.

        :param dr_value: The value of the selection dropdown for Dimensionality Reduction algos.(param.parameterized.Parameter)
        :param clustering_value: The value of the selection dropdown for Clustering algos.(param.parameterized.Parameter)
        :return: A similar events configuration widgetbox.
    ...
    def build_event_page(
            ...
            clusters):
        """
        The method build the event page that depends on the data-exploraton pane.

        :param selection: The details of selected point in the data exploration pane.(param.list)
        :param selected_df:A dataframe contains the file name of input event, event id, start time, asset name
        and data for x and y axis.(pandas.core.frame.DataFrame)
        :param x_axis: The value of x axis dropdown widget.(param.parameterized.Parameter)
        :param y_axis: The value of y axis dropdown widget.(param.parameterized.Parameter)
        :param clusters: The value of clustering selection dropdown widget.(param.parameterized.Parameter).
        :return:The layout of selected event page.
        ...
        def identify_top_predictions(df, event_id):
            """
            The method build a helper function to identify the top PQD predictions for the waveforms.

            :param df:The dataframe contains data from input file.(pandas.core.frame.DataFrame)
            :param event_id: The id of selected event.(string)
            :return: Return a dictionary with the waveform names as keys, and their top PQD types and scores as values.
    ...
    def build_similar_event_page(
        ...
        """
        The method build the interactive similar events page that depends on the data-exploraton pane.
        ...
        :param y_axis: The value of y axis dropdown widget.(param.parameterized.Parameter)
        :param clusters: The value of clustering selection dropdown widget.(param.parameterized.Parameter).
        :param similar_events_x_parameter: The value of the FloatSlider which controls elected event's x-axis prediction
         score for no dimensionality algos for no clustering algos.(float)
        :param similar_events_y_parameter: The value of the FloatSlider which controls elected event's x-axis prediction
         score for no dimensionality algos for no clustering algos.(float)
        :return:The layout of similar event page.
        ...
        def _download_callback():
            """
            A callback when the download button is clicked.

            :return: An object to output.(_io.StringIO)
        ...
        def build_similar_events(_):
            """
            If the similar events button is clicked on the similar events summary Tabulator, the methods will build similar
             events which contains details of metadata, PQD predictions, current waveforms, voltage waveforms, etc.

            :return:All selected similar events details.
            """
            similar_events_selection = similar_events_tabs.selection
    ...
    def data_exploration(dr_value, clustering_value,
        ...
        """
        The method build the interactive data-exploration pane, based on the selection of plot configuration.
        The parameters of the method are values of plot configuration widgets.

        :return: A interactive data-exploration pane.
        ...
        if dr_value == "Nil" and clustering_value == "Nil":
            ...
            def get_event(selection):
                """
                If users select a point of data exploration pane, the method will be called.

                :param selection: The event id of the selected event.(param.List)
                :return: The selected event page.
            ...
            def get_similar_events(selection):
                ...
                :param selection: The event id of the selected event.(param.List)
                :return: The similar events page.
